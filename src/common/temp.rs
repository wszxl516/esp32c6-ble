...
use esp_idf_svc::sys::{esp, temperature_sensor_abs_threshold_config_t,
                       temperature_sensor_config_t, temperature_sensor_enable,
                       temperature_sensor_get_celsius, temperature_sensor_install,
...
impl Tmep{
    pub fn new() -> anyhow::Result<Self>{
        ...
        unsafe {esp!(temperature_sensor_install(addr_of!(temp_sensor_config) ,  addr_of_mut!(temp_sensor)))}?;
        let threshold_cfg =  temperature_sensor_abs_threshold_config_t{
            high_threshold: 50.0,
            low_threshold: 0.0,
        };
        unsafe {esp!(temperature_sensor_set_absolute_threshold(temp_sensor, addr_of!(threshold_cfg)))}?;
        unsafe {temperature_sensor_enable(temp_sensor)};
        Ok(Self{sensor: temp_sensor})
    }
